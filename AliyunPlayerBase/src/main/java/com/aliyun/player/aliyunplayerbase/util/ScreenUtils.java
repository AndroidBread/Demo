...
//    }
    ...
    /***
     * 隐藏系统状态栏和导航栏
     * @param activity 对应的活动页面
     */
    public static void hideSystemBars(Activity activity){
        WindowInsetsControllerCompat controllerCompat = new WindowInsetsControllerCompat(activity.getWindow() , activity.getWindow().getDecorView());
        controllerCompat.setSystemBarsBehavior(BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE);
        controllerCompat.hide(systemBars());
    }
    ...
    public static void showSystemBars(Activity activity){
        WindowInsetsControllerCompat controllerCompat = new WindowInsetsControllerCompat(activity.getWindow() , activity.getWindow().getDecorView());
        controllerCompat.show(systemBars());
    }
