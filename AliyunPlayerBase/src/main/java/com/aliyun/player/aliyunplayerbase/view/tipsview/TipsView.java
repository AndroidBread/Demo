...
//    private Theme mCurrentTheme;
    ...
    public void setScreenModeStatus(AliyunScreenMode mode) {
        if(mReplayView != null){
            mReplayView.setScreenModeStatus(mode);
        }
    }
     ...
     * @param msg   错误信息
     */
    public void showErrorTipViewWithoutCode(boolean needRetry, String msg) {
        if (mErrorView == null) {
            mErrorView = new ErrorView(getContext());
            mErrorView.updateTipsWithoutCode(needRetry , msg);
            mErrorView.setOnBackClickListener(onTipsViewBackClickListener);
            mErrorView.setOnRetryClickListener(onRetryClickListener);
            addSubView(mErrorView);
        }
